...
router.get("/clinics", async (req, res) => {
  try {
    ...
    // Obtener datos adicionales para cada clínica (fotos, reviews, servicios, horarios)
    const clinicasEnriquecidas = await Promise.all(
      clinicas.map(async (clinica) => {
        ...
        if (errorFotos) {
          console.error(
            `Error obteniendo fotos para clínica ${clinica.id_clinica}:`,
            errorFotos,
          );
        }

        // 2. Obtener reseñas para calcular rating promedio
        const { data: reviews, error: errorReviews } = await supabaseClient
          .from("reseñas")
          .select("calificacion")
          .eq("id_clinica", clinica.id_clinica);
        ...
        if (!errorReviews && reviews && reviews.length > 0) {
          ...
          const totalRating = reviews.reduce(
            (sum, review) => sum + review.calificacion,
            0,
          );
          averageRating = totalRating / reviewCount;
        }

        // 3. Obtener categorías únicas de servicios disponibles
        const { data: servicios, error: errorServicios } = await supabaseClient
          .from("servicios")
          .select("categoria, disponible")
          .eq("id_clinica", clinica.id_clinica)
          .eq("disponible", true);
        ...
        if (!errorServicios && servicios) {
          ...
          serviceCategories.push(...uniqueCategories.filter((cat) => cat)); // Filter out null/undefined
        }
        ...
        const { data: horarios, error: errorHorarios } = await supabaseClient
          .from("horarios_atencion")
          .select(
            "dia_semana, hora_apertura, hora_cierre, es_24h, esta_cerrado",
          )
          .eq("id_clinica", clinica.id_clinica);
        ...
        if (!errorHorarios && horarios) {
          ...
          horarios.forEach((horario) => {
            const dayKey =
              dayMap[horario.dia_semana?.toLowerCase()] ||
              horario.dia_semana?.toLowerCase();
            if (dayKey) {
              openingHours[dayKey] = {
                open: horario.hora_apertura,
                close: horario.hora_cierre,
                is24Hours: horario.es_24h || false,
                closed: horario.esta_cerrado || false,
              };
            }
          });
        }
        ...
        return {
          ...
          serviceCategories,
          openingHours,
          // Extraer datos de detalles para facilitar el acceso
          specialties: clinica.detalles?.especialidades || [],
          facilities: clinica.detalles?.instalaciones || [],
          paymentMethods: clinica.detalles?.metodos_pago || [],
        };
      }),
    );
